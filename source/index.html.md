...
toc_footers:
  - <a href='https://GulfEx.com/account/developers'>Sign Up for a Developer Key</a>
...
GulfEx provides a complete RESTful API for developers which allows full access to all the functionalities on the exchange. GulfEx also support websocket for real-time updates.
...
Client libraries make it simple to utilize our API. Currently, there are two libraries for GulfEx that support three languages:

- [GulfEx Node Library](https://github.com/bitholla/GulfEx-node-lib) - Our official library that supports Node.js. Connects to both our API and websocket.
...
```shell
curl -X GET "https://api.GulfEx.com/v2/health"
```
...
`GET https://api.GulfEx.com/v2/health`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/constants"
```
...
```json
{
    "coins": {
        "xht": {
            ...
            "logo": "https://bitholla.s3.ap-northeast-2.amazonaws.com/icon/XHT-GulfEx-asset-01.svg",
            "code": "xht",
            "is_public": true,
            "meta": {},
...
`GET https://api.GulfEx.com/v2/constants`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/kit"
```
...
```json
{
    ...
    "user_meta": {...},
    "logo_image": "https://bitholla-sandbox.s3.ap-northeast-2.amazonaws.com/exchange/Sandbox_GulfEx/EXCHANGE_LOGO__dark.png",
    "description": "GulfEx is a global cryptocurrency exchange for professional trading built based on GulfEx Kit technology developed and managed by bitHolla. Not only it is open for trading but it also allows businesses and individuals to branch out and create their own exchange based on it in the GulfEx Network. GulfEx Token (XHT) is the native token of GulfEx Network used as a collateral among exchanges.",
    "injected_html": {...},
    "injected_values": [...],
...
`GET https://api.GulfEx.com/v2/kit`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/tiers"
```
...
`GET https://api.GulfEx.com/v2/tiers`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/ticker?symbol=xht-usdt"
```
...
`GET https://api.GulfEx.com/v2/ticker?symbol=${symbol}`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/tickers"
```
...
`GET https://api.GulfEx.com/v2/tickers`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/orderbook?symbol=xht-usdt"
```
...
`GET https://api.GulfEx.com/v2/orderbook?symbol=${symbol}`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/orderbooks"
```
...
`GET https://api.GulfEx.com/v2/orderbooks`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/trades?symbol=xht-usdt"
```
...
`GET https://api.GulfEx.com/v2/trades`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/chart?symbol=xht-usdt&resolution=1D&from=1616987453&to=1619579513"
```
...
`GET https://api.GulfEx.com/v2/chart?symbol=${symbol}&resolution=${resolution}&from=${from}&to=${to}`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/charts?resolution=1D&from=1551663947&to=1582768007"
```
...
`GET https://api.GulfEx.com/v2/charts?resolution=${resolution}&from=${from}&to=${to}`
...
`GET https://api.GulfEx.com/v2/user`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/user/balance"
```
...
`GET https://api.GulfEx.com/v2/user/balance`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/user/deposits"
```
...
`GET https://api.GulfEx.com/v2/user/deposits`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/user/withdrawals"
```
...
`GET https://api.GulfEx.com/v2/user/withdrawals`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/user/withdrawal/fee?symbol=$symbol"
```
...
`GET https://api.GulfEx.com/v2/user/withdrawal/fee?currency=${currency}`
...
curl -X POST
  ...
  "https://api.GulfEx.com/v2/user/withdrawal"
```
...
`POST https://api.GulfEx.com/v2/user/withdrawal`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/user/trades"
```
...
`GET https://api.GulfEx.com/v2/user/trades`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/orders"
```
...
`GET https://api.GulfEx.com/v2/orders`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/order?$order_id"
```
...
`GET https://api.GulfEx.com/v2/order?order_id=${order_id}`
...
`POST https://api.GulfEx.com/v2/order`
...
curl -X DELETE
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/order/all?symbol=xht-usdt"
```
...
`DELETE https://api.GulfEx.com/v2/order/all`
...
curl -X GET
  ...
  -H "api-signature: $API_SIGNATURE"
  -H "api-expires: $API_EXPIRES"
  "https://api.GulfEx.com/v2/order?order_id=$order_id"
```
...
`DELETE https://api.GulfEx.com/v2/order?order_id=${order_id}`
...
GulfEx fully supports the TradingView UDF API.
...
```shell
curl -X GET "https://api.GulfEx.com/v2/udf/config"
```
...
`GET https://api.GulfEx.com/v2/udf/config`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/udf/history?symbol=xht-usdt&resolution=1D&from=1551663947&to=1582768007"
```
...
`GET https://api.GulfEx.com/v2/udf/history?symbol=${symbol}&resolution=${resolution}&from=${from}&to=${to}`
...
```shell
curl -X GET "https://api.GulfEx.com/v2/udf/symbols?symbol=xht-usdt"
```
...
`GET https://api.GulfEx.com/v2/udf/symbols?symbol=${symbol}`
...
Connection to public and private channels are available through the path `https://api.GulfEx.com/stream`. GulfEx Exchanges can be connected to using any websocket libraries. For this documentation, we will use the library [`ws`]('https://github.com/websockets/ws').
...
```javascript
	...
	// Public
	const client = new WebSocket('https://api.GulfEx.com/stream');

	// Private Bearer
	const client = new WebSocket(`https://api.GulfEx.com/stream?authorization=Bearer%20${TOKEN}`);

	// Private HMAC
	const client = new WebSocket(`https://api.GulfEx.com/stream?api-key=${API-KEY}&api-signature=${API-SIGNATURE}&api-expires=${API-EXPIRES}`);
